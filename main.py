...
def parse_args():
    ...
    # Convolutional parameters for the encoder
    parser.add_argument('--conv_kernel_size', default=3, type=int, help='Kernel size for the convolutional layer in the encoder')
    parser.add_argument('--conv_stride', default=1, type=int, help='Stride for the convolutional layer in the encoder')
    parser.add_argument('--conv_padding', default=1, type=int, help='Padding for the convolutional layer in the encoder')
...
def test(args, model):
    ...
    for inputs, tgt_input, label in test_dataloader:
        ...
        # 解码过程
        with torch.no_grad():
            preds = model(src, tgt_input.cuda(), src_mask, tgt_mask) # [bs, n, 7]

        
            # import pdb; pdb.set_trace()
        loss = F.mse_loss(preds, label.cuda())
        loss2 = F.l1_loss(preds, label.cuda())
        # import pdb; pdb.set_trace()
        outputs = preds
        ...
        log.info(f'time = {int((time.time() - start) // 60)}m, mse_loss = {loss:.5f}, mae_loss = {loss2:.5f}')

    loss_avg = total_loss / len(test_dataloader)
    log.info(f'time = {int((time.time() - start) // 60)}m, mse_loss = {loss_avg:.5f}, mae_loss = {loss2:.5f}')
    return

def test_teacher(args, model):
    log = init_logger(args.log_file, mode='w')
...
def draw(args, npList, label):
    ...
    for i in range(7):
        ...
        #plt.plot(x[args.output_size:], prediction_list[i], label='Prediction')
